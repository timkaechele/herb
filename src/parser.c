...
void herb_parser_init(parser_T* parser, lexer_T* lexer, parser_options_T options) {
  ...
  hb_array_pointer_init(&parser->open_tags_stack, 16);
...
static AST_CDATA_NODE_T* parser_parse_cdata(parser_T* parser) {
  hb_array_T errors;
  hb_array_pointer_init(&errors, 8);
  hb_array_T children;
...
static hb_array_T parser_build_elements_from_tags(hb_array_T* nodes, hb_array_T* errors) {
  hb_array_T result;
  hb_array_pointer_init(&result, nodes->size);
  ...
  for (size_t index = 0; index < nodes->size; index++) {
    ...
    if (node->type == AST_HTML_OPEN_TAG_NODE) {
      ...
      if (close_index == (size_t) -1) {
        ...
        hb_array_T body;
        hb_array_pointer_init(&body, close_index - index - 1);
        ...
        hb_array_deinit(&body);

        hb_array_T element_errors;
        hb_array_pointer_init(&element_errors, 8);
...
static AST_DOCUMENT_NODE_T* parser_parse_document(parser_T* parser) {
  hb_array_T children; // TODO(Tim) check if this is correct
  hb_array_pointer_init(&children, 8);
  hb_array_T errors;
  hb_array_pointer_init(&errors, 8);
  position_T start = parser->current_token->location.start;
...
void herb_parser_deinit(parser_T* parser) {
  ...
  hb_array_deinit(&parser->open_tags_stack);
}
